...
class StyledFormMixin:
    """ Mixin to apply Tailwind styles to form fields """
    default_classes = "border-2 border-gray-300 w-full p-3 rounded-lg shadow-sm focus:outline-none focus:border-rose-500 focus:ring-rose-500"

    def apply_styled_widgets(self):
        for field_name, field in self.fields.items():
            if isinstance(field.widget, forms.TextInput) or isinstance(field.widget, forms.EmailInput) or isinstance(field.widget, forms.TimeInput) or isinstance(field.widget, forms.Select):
            ...
            elif isinstance(field.widget, forms.SelectDateWidget) or isinstance(field.widget, forms.DateInput):
                field.widget.attrs.update({
